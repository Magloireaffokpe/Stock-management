...
class ProductDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        """Supprime définitivement le produit ainsi que ses traces dépendantes."""
        instance = self.get_object()
        product_name = instance.name

        from django.db import transaction
        from apps.sales.models import SaleItem, QuotationItem, RestockItem
        from apps.stock.models import StockMovement, StockAlert
        from apps.settings_app.utils import log_audit_action

        with transaction.atomic():
            SaleItem.objects.filter(product=instance).delete()
            QuotationItem.objects.filter(product=instance).delete()
            RestockItem.objects.filter(product=instance).delete()
...
class ProductSearchView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .filter(models.Q(name__icontains=q) | models.Q(sku__icontains=q))
