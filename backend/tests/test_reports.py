...
class DashboardKPITest(BaseTestCase):
    ...
    def test_accessible_tous_connectés(self):
        r = self.employee_client.get('/api/reports/dashboard/')
    ...
    def test_structure_réponse_complète(self):
        ...
        for champ in ['revenue', 'profit', 'count', 'variation_revenue', 'variation_count']:
            self.assertIn(champ, r.data['today'])

        # Vérifier les champs dans 'stock'
        for champ in ['total_products', 'out_of_stock', 'critical', 'low', 'unread_alerts']:
    ...
    def test_kpi_today_reflète_vente_du_jour(self):
        ...
        make_sale(self.admin, items=[{'product': p, 'quantity': 1, 'unit_price': 75000}])
    ...
    def test_kpi_exclut_ventes_annulées(self):
        ...
        sale = make_sale(self.admin, items=[{'product': p, 'quantity': 1, 'unit_price': 100000}])
    ...
    def test_kpi_today_profit_calculé(self):
        ...
        make_sale(self.admin, items=[{'product': p, 'quantity': 2, 'unit_price': 50000}])
...
class DailySalesChartTest(BaseTestCase):
    ...
    def test_accessible_employé(self):
        r = self.employee_client.get('/api/reports/charts/daily/')
        self.assertEqual(r.status_code, status.HTTP_200_OK)
...
class CategorySalesChartTest(BaseTestCase):
    ...
    def test_retourne_catégories_avec_ventes(self):
        ...
        p = make_product(name='Prod Chart', category=cat, stock_quantity=10, selling_price=20000)
        make_sale(self.admin, items=[{'product': p, 'quantity': 1, 'unit_price': 20000}])
...
class PaymentMethodStatsTest(BaseTestCase):
    ...
    def test_méthodes_paiement_représentées(self):
        ...
        make_sale(self.admin, items=[{'product': p1, 'quantity': 1, 'unit_price': 10000}],
                  payment_method='cash')
        make_sale(self.admin, items=[{'product': p2, 'quantity': 1, 'unit_price': 10000}],
                  payment_method='mtn')
        r = self.admin_client.get('/api/reports/charts/payment-methods/')
...
class TopProductsTest(BaseTestCase):
    ...
    def test_accessible(self):
        r = self.admin_client.get('/api/reports/top-products/')
        self.assertEqual(r.status_code, status.HTTP_200_OK)

    def test_top_produit_trié_par_ca(self):
        p_gros = make_product(name='Top Vendeur', selling_price=100000, stock_quantity=20)
        p_petit = make_product(name='Petit Vendeur', selling_price=5000, stock_quantity=20)
        make_sale(self.admin, items=[{'product': p_gros, 'quantity': 3, 'unit_price': 100000}])
        make_sale(self.admin, items=[{'product': p_petit, 'quantity': 1, 'unit_price': 5000}])
    ...
    def test_limite_nombre_résultats(self):
        for i in range(12):
            ...
            make_sale(self.admin, items=[{'product': p, 'quantity': 1, 'unit_price': 5000}])
...
class StockValueTest(BaseTestCase):
    ...
    def test_structure_réponse(self):
        r = self.admin_client.get('/api/reports/stock-value/')
        for champ in ['purchase_value', 'selling_value', 'total_products',
                      'total_units', 'potential_profit']:
    ...
    def test_potential_profit_égal_différence(self):
        r = self.admin_client.get('/api/reports/stock-value/')
        diff = Decimal(str(r.data['selling_value'])) - Decimal(str(r.data['purchase_value']))
...
class RecentSalesTest(BaseTestCase):
    ...
    def test_ventes_triées_plus_récentes_en_premier(self):
        ...
        s1 = make_sale(self.admin, items=[{'product': p, 'quantity': 1, 'unit_price': 10000}])
        s2 = make_sale(self.admin, items=[{'product': p, 'quantity': 1, 'unit_price': 10000}])
        r = self.admin_client.get('/api/reports/dashboard/recent-sales/')
...
class ExcelExportTest(BaseTestCase):
    ...
    def test_export_ventes_avec_filtre_date(self):
        ...
        r = self.admin_client.get(f'/api/reports/export/sales/?date_from={today}&date_to={today}')
        self.assertEqual(r.status_code, status.HTTP_200_OK)
